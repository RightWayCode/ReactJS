...
import { getCarts, removeCart } from "../redux/action/cart";
...
const Cart = () => {
  const getCart = useSelector((state) => state.cartData.carts);
  const dispatch = useDispatch();
  const [cart, setCart] = useState(5);

  useEffect(() => {
    dispatch(getCarts(cart));
    console.log("main cart data: ", getCart);
  }, [cart]);
  ...
  const total = getCart.reduce((sum, item) => sum + item.price, 0).toFixed(2);
